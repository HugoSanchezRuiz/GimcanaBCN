...
<body>
    ...
    <div class="column-2">
        ...
        <div class="container">
            ...
            <div class="container" id="createTipoUbicacionContainer" style="display: none;">
                ...
                <form id="createTipoUbicacionForm" style="background-color: #3498db; color: #fff; padding: 20px; border-radius: 5px;">
                    ...
                    <input type="text" id="nombre" name="nombre" style="padding: 8px; margin-bottom: 10px; width: 100%; box-sizing: border-box;">
            
                    <label for="logo" style="display: block; margin-bottom: 10px;">Logo:</label>
                    <input type="file" id="logo" name="logo" style="padding: 8px; margin-bottom: 10px; width: 100%; box-sizing: border-box;">
        ...
        <br>
        <button id="crear">Crear gimcana</button>
        <br>
        <br>
        <div class="container" id="createGimcanaContainer" style="display: none;">
            <form action="" id="formulario">
                @csrf
                <!-- Primer select -->
                <div>
                    <label for="ubicacion1">Ubicación 1:</label>
                    <select id="ubicacion1" name="ubicacion1"></select>
                </div>

                <!-- Segundo select -->
                <div>
                    <label for="ubicacion2">Ubicación 2:</label>
                    <select id="ubicacion2" name="ubicacion2"></select>
                </div>

                <!-- Tercer select -->
                <div>
                    <label for="ubicacion3">Ubicación 3:</label>
                    <select id="ubicacion3" name="ubicacion3"></select>
                </div>

                <button id="btn-crear-gimcana" type="submit">Crear gimcana</button>
            </form>
        </div>
        ...
        <h1>Gimcanas</h1>
        <br>
        <div id="gimcana-container"></div>

        
    </div>
    <div class="column-3" id="map"></div>
    </body>
    ...
    <script>
        function initMap() {
            ...
            @foreach ($ubicaciones as $ubicacion)
                ...
                @endphp
                addMarker({{ $ubicacion->latitud }}, {{ $ubicacion->longitud }}, '{{ $ubicacion->nombre }}',
                    '{{ asset("img/$ubicacion->imagen") }}', '{{ $logoUrl }}', '{{ $ubicacion->ciudad }}',
                    '{{ $ubicacion->calle }}', '{{ $ubicacion->num_calle }}', '{{ $ubicacion->codigo_postal }}',
                    '{{ $ubicacion->descripcion }}', '{{ $tipoUbicacion ? $tipoUbicacion->nombre : 'Sin tipo' }}'); @endforeach



            function addMarker(lat, lng, nombre, imagenUbicacion, imagenTipoUbicacion, ciudad, calle, numCalle,
                codigoPostal, descripcion, tipo_ubicacion_id) {
                var ubicacionMarker;
    ...
    '<img src="' + imagenUbicacion + '" alt="Imagen de ubicación" height="150px" width="150px">' +
    '<p><strong style="color: black;">Ciudad:</strong> <span style="color: grey;">' + ciudad +
            '</span></p>' +
    '<p><strong style="color: black;">Calle:</strong> <span style="color: grey;">' + calle + '</span></p>' +
    '<p><strong style="color: black;">Número de Calle:</strong> <span style="color: grey;">' + numCalle +
            '</span></p>' +
    '<p><strong style="color: black;">Código Postal:</strong> <span style="color: grey;">' + codigoPostal +
            '</span></p>' +
    '<p><strong style="color: black;">Categoría:</strong><span style="color: grey;">' + tipo_ubicacion_id +
            '</span></p>' +
    ...
    `;
                    ...
                    const uniqueLocationTypes = [...new Set(data.map(location => location.tipo_ubicacion && location
                    ...
                    function applyFilters() {
                        ...
                        data.forEach(location => {
                            ...
                            const address = `${location.calle}, ${location.num_calle}, ${location.ciudad}`
                                .toLowerCase();
                            const locationType = location.tipo_ubicacion && location.tipo_ubicacion.nombre ?
                                location.tipo_ubicacion.nombre.toLowerCase() : '';

                            if (name.includes(filterName) && address.includes(filterAddress) && locationType
                                .includes(filterLocationType)) {
                                row.style.display = '';
                            } else {
    ...
    `;
        ...
        function fetchGimcana() {
            fetch('/get-gimcana')
                .then(response => response.json())
                .then(data => {
                    const gimcanaContainer = document.getElementById('gimcana-container');
                    gimcanaContainer.innerHTML = '';

                    // Verificar si 'data' tiene las propiedades 'gimcana' y 'ubicaciones'
                    if (data.gimcana && data.ubicaciones) {
                        // Iterar sobre las gimcanas
                        data.gimcana.forEach((gimcana, index) => {
                            // Crear un elemento div para representar una gimcana
                            const gimcanaDiv = document.createElement('div');
                            gimcanaDiv.classList.add('gimcana');

                            // Crear un encabezado con el nombre de la gimcana
                            const gimcanaHeader = document.createElement('h2');
                            gimcanaHeader.textContent = gimcana.nombre_gimcana;
                            gimcanaDiv.appendChild(gimcanaHeader);

                            // Crear el botón de borrar la gimcana
                            const deleteButton = document.createElement('button');
                            deleteButton.textContent = 'Borrar Gimcana';
                            deleteButton.addEventListener('click', () => deleteGimcana(gimcana.id));
                            gimcanaDiv.appendChild(deleteButton);

                            // Crear una lista ul para las ubicaciones de la gimcana
                            const ubicacionesList = document.createElement('ul');

                            // Iterar sobre las ubicaciones asociadas a esta gimcana
                            data.ubicaciones[gimcana.id].forEach(nombreUbicacion => {
                                // Crear un elemento de lista li para cada ubicación
                                const ubicacionItem = document.createElement('li');
                                ubicacionItem.textContent = nombreUbicacion;
                                ubicacionesList.appendChild(ubicacionItem);
                            });

                            // Agregar la lista de ubicaciones al div de la gimcana
                            gimcanaDiv.appendChild(ubicacionesList);

                            // Agregar el div de la gimcana al contenedor principal
                            gimcanaContainer.appendChild(gimcanaDiv);

                            // Agregar un elemento de separación si no es la última gimcana
                            if (index < data.gimcana.length - 1) {
                                const separationElement = document.createElement('hr');
                                gimcanaContainer.appendChild(separationElement);
                            }
                        });
                    } else {
                ...
                .catch(error => {
                    console.error('Error fetching gimcana:', error);
                });
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Obtener el formulario y los selectores de ubicación
            const formulario = document.getElementById('formulario');
            const ubicacion1Select = document.getElementById('ubicacion1');
            const ubicacion2Select = document.getElementById('ubicacion2');
            const ubicacion3Select = document.getElementById('ubicacion3');

            // Añadir un evento de escucha al formulario para manejar el envío
            formulario.addEventListener('submit', function(event) {
                event.preventDefault(); // Evitar el envío del formulario por defecto

                const ubicacion1 = ubicacion1Select.value;
                const ubicacion2 = ubicacion2Select.value;
                const ubicacion3 = ubicacion3Select.value;

                // Validar que se haya seleccionado al menos una ubicación
                if (!ubicacion1 || !ubicacion2 || !ubicacion3 || ubicacion1 === '' || ubicacion2 === '' ||
                    ubicacion3 === '') {
                    swal({
                        title: 'Error',
                        text: 'Por favor, selecciona tres ubicaciones para los tres selectores.',
                        icon: 'error',
                        button: 'Entendido'
                    });
                    return false;
                ...
                // Validar que no se hayan seleccionado las mismas ubicaciones
                if (ubicacion1 === ubicacion2 || ubicacion1 === ubicacion3 || ubicacion2 === ubicacion3) {
                    swal({
                        title: 'Error',
                        text: 'Por favor, selecciona ubicaciones diferentes para cada selector.',
                        icon: 'error',
                        button: 'Entendido'
                    });
                    return false;
                ...
                fetch('/gimcana/create', {
                    ...
                    .then(response => {
                        if (response.ok) {
                            Swal.fire({
                                title: '¡Gimcana creada!',
                                text: 'La gimcana se ha creado exitosamente.',
                                icon: 'success',
                                confirmButtonText: 'Entendido'
                            }).then((result) => {
                                // Puedes agregar aquí alguna lógica adicional después de que se cierre el SweetAlert
                            ...
                            alert('La gimcana no se ha creado exitosamente.');
                            console.error('Error al crear gimcana:', response.statusText);
                        }
                    })
                    .catch(error => {
            ...
            // Hacer una solicitud para obtener las ubicaciones disponibles
            fetch('/get-ubicaciones')
                .then(response => response.json())
                .then(data => {
                    // Agregar la opción "Selecciona una ubicación" a cada select
                    [ubicacion1Select, ubicacion2Select, ubicacion3Select].forEach(select => {
                    ...
                    // Llenar los selects con opciones de ubicaciones
                    data.forEach(ubicacion => {
                        const option = document.createElement('option');
                        option.value = ubicacion.id;
                        option.textContent = ubicacion.nombre;
                        ubicacion1Select.appendChild(option.cloneNode(true));
                        ubicacion2Select.appendChild(option.cloneNode(true));
                        ubicacion3Select.appendChild(option.cloneNode(true));
                    });
                })
                .catch(error => {
        ...
        function openEditLocationModal(locationData, tipoUbicaciones) {
            ...
            const tipoUbicacionesOptions = tipoUbicaciones.map(tipoUbicacion =>
                `<option value="${tipoUbicacion.id}">${tipoUbicacion.nombre}</option>`);
...
<button type="submit" class="btn btn-primary btn-update">Actualizar</button>
                ...
                if (result.isConfirmed) {
                    ...
                    xhr.onload = function() {
                        if (xhr.status === 200) {
                            ...
                            console.error('Error al eliminar la ubicación:', errorMessage);
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: errorMessage
                    ...
                    xhr.onload = function() {
                        if (xhr.status === 200) {
                            ...
                            Swal.fire({
                                ...
                                // Recargar las ubicaciones después de la eliminación exitosa
                                fetchTipoUbicaciones();
                                fetchLocations();
                            });
                        } else {
        ...
        function deleteGimcana(gimcanaId) {
            Swal.fire({
                title: `¿Estás seguro de que quieres borrar la gimcana con ID ${gimcanaId}?`,
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.isConfirmed) {
                    var csrftoken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
                    var xhr = new XMLHttpRequest();
                    xhr.open('DELETE', `/gimcana/${gimcanaId}`, true);
                    xhr.setRequestHeader('X-CSRF-TOKEN', csrftoken);
                    xhr.onload = function() {
                        if (xhr.status === 200) {
                            // La eliminación fue exitosa
                            ...
                            Swal.fire({
                                ...
                                // Recargar las gimcanas después de la eliminación exitosa
                                fetchGimcana();
                            });
                        } else {
                            // Si la respuesta no es exitosa, mostrar un mensaje de error
                            console.error('Error al eliminar la gimcana:', xhr.statusText);
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'Error al eliminar la gimcana'
                            });
                        }
                    };
                    xhr.onerror = function() {
...
<form id="editTipoUbicacionForm">
    ...
    <button type="submit" class="btn btn-primary">Actualizar</button>
            ...
            Swal.fire({
                ...
                onOpen: function() {
                    // Manejar el envío del formulario cuando se abra Sweet Alert
                    document.getElementById('editTipoUbicacionForm').addEventListener('submit', function(
                        event) {
                        event.preventDefault();
                        // Obtener los datos del formulario
                        var formData = new FormData(this);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const crearTipoUbicacionBtn = document.getElementById('crear');
            const createTipoUbicacionContainer = document.getElementById('createGimcanaContainer');
